...
//
//  MC_holePropagationInfiniteSpin.cpp
//  
//
...
void holePropagationInfiniteSpin::run(void) {
...
void holePropagationInfiniteSpin::run(const pathContainer &pC) {
    int iSite;                          // site index
    set<pathData>::iterator it;         // iterator of pathData elements within pC[iSite]
    ...
    for(iSite=0; iSite<(this->nSites); iSite++) {
        for(it=pC[iSite].begin(); it!=pC[iSite].end(); ++it) {
            contribution = conj((*it).getCoefficient()) * (*it).getCoefficient();
...
// (Overloaded function!)
bool holePropagationInfiniteSpin::loadParameters(void) {
...
void holePropagationInfiniteSpin::generateSaveFilenames() {
    stringstream fnameBaseStream;
    string fnameBaseNew;
    
    // Create the base of all the file names
    fnameBaseStream << "InfiniteT_InfiniteSpin_ver1_10_t=" << setprecision(2) << fixed << this->tHole;
...
// (Overloaded function!)
bool holePropagationInfiniteSpin::saveParameters(void) const {
    string fname = this->saveFilenames[1];
    ofstream myfile (fname);
    if(!myfile) {
        cerr << "Error in holePropagationInfiniteSpin::saveParameters writing to " << fname << endl << endl;
        return false;
    }
